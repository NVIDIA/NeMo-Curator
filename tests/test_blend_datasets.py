...
class TestBlending:
    def test_blend_as_original(self) -> None:
    ...
    def test_equal_blend(self) -> None:
        ...
        assert len(result_dataset) == 2  # noqa: PLR2004
        assert counts["a"] == 1
        assert counts["b"] == 1

    def test_equal_blend_with_weights(self) -> None:
        first_dataset = list_to_dataset(["a", "a"])
        second_dataset = list_to_dataset(["b", "b"])
    ...
    def test_uneven_blend(self) -> None:
        first_dataset = list_to_dataset(["a", "a"])
        second_dataset = list_to_dataset(["b", "b"])
        result_dataset = nc.blend_datasets(4, [first_dataset, second_dataset], [3.0, 1.0])
        counts = result_dataset.df["text"].value_counts().compute()
        assert len(result_dataset) == 4  # noqa: PLR2004
        assert counts["a"] == 3  # noqa: PLR2004
        assert counts["b"] == 1

    def test_very_uneven_blend(self) -> None:
        first_dataset = list_to_dataset(["a", "a"])
        second_dataset = list_to_dataset(["b", "b"])
        result_dataset = nc.blend_datasets(4, [first_dataset, second_dataset], [1.0, 0.0])
        counts = result_dataset.df["text"].value_counts().compute()
        assert len(result_dataset) == 4  # noqa: PLR2004
        assert counts["a"] == 4  # noqa: PLR2004
        assert "b" not in counts

    def test_proper_uneven_blend(self) -> None:
        ...
        counts = result_dataset.df["text"].value_counts().compute()
        assert len(result_dataset) == 8  # noqa: PLR2004
        assert counts["a"] == 2  # noqa: PLR2004
        assert counts["b"] == 2  # noqa: PLR2004
        assert counts["c"] == 2  # noqa: PLR2004
        assert counts["d"] == 2  # noqa: PLR2004

    def test_four_dataset_blend(self) -> None:
        datasets = []
        ...
        counts = result_dataset.df["text"].value_counts().compute()
        assert len(result_dataset) == 10  # noqa: PLR2004
        assert counts["a"] == 1
        assert counts["b"] == 2  # noqa: PLR2004
        assert counts["c"] == 3  # noqa: PLR2004
        assert counts["d"] == 4  # noqa: PLR2004
