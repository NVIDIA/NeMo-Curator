...
class TestArgumentHelper:
    ...
    def test_add_arg_output_file_type(self) -> None:
        parser = argparse.ArgumentParser()
        argument_helper = ArgumentHelper(parser)
        choices = ["jsonl", "pickle", "parquet"]
    ...
    def test_set_default_n_workers(self) -> None:
        parser = argparse.ArgumentParser()
        argument_helper = ArgumentHelper(parser)
        max_mem_gb_per_worker = 10.0
        argument_helper.set_default_n_workers(max_mem_gb_per_worker)
        assert "n_workers" in argument_helper.parser._defaults  # noqa: SLF001

    def test_parse_client_args(self) -> None:
        parser = argparse.ArgumentParser()
        argument_helper = ArgumentHelper(parser)
        ...
        assert parsed_args["n_workers"] == 10  # noqa: PLR2004
        assert "random_arg" not in parsed_args

    def test_parse_distributed_classifier_args(self) -> None:
        parser = argparse.ArgumentParser()
        argument_helper = ArgumentHelper(parser)
        ...
        assert "rmm_pool_size" in parser._defaults  # noqa: SLF001
        assert "set_torch_to_use_rmm" in parser._defaults  # noqa: SLF001
    ...
    def test_add_distributed_classifier_cluster_args(self) -> None:
        parser = argparse.ArgumentParser()
        argument_helper = ArgumentHelper(parser)
        ...
        assert "--set-torch-to-use-rmm" in argument_helper.parser.format_help()
        assert "--max-mem-gb-classifier" in argument_helper.parser.format_help()
        assert "rmm_pool_size" in argument_helper.parser._defaults  # noqa: SLF001
        assert "set_torch_to_use_rmm" in argument_helper.parser._defaults  # noqa: SLF001

    def test_parse_gpu_dedup_args(self) -> None:
        parser = argparse.ArgumentParser()
        argument_helper = ArgumentHelper(parser)
        ...
        assert "device" in argument_helper.parser._defaults  # noqa: SLF001
        assert "set_torch_to_use_rmm" in argument_helper.parser._defaults  # noqa: SLF001
    ...
    def test_parse_semdedup_args(self) -> None:
        parser = argparse.ArgumentParser()
        argument_helper = ArgumentHelper(parser)
        ...
        assert "rmm_pool_size" in parser._defaults  # noqa: SLF001
        assert "device" in parser._defaults  # noqa: SLF001
        assert "set_torch_to_use_rmm" in parser._defaults  # noqa: SLF001
