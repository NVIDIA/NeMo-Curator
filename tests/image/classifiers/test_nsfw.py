...
Normalization = gpu_only_import_from("nemo_curator.image.classifiers.nsfw", "Normalization")
...
def test_get_default_model() -> None:
    ...
    with (
        ...
        model_path = NsfwClassifier._get_default_model()  # noqa: SLF001
...
def test_classifier_with_embedder_workflow(gpu_client) -> None:  # noqa: ANN001, ARG001
    ...
    def mock_load_model(device: torch.device) -> Callable:  # noqa: ARG001
    ...
    def mock_load_object(name: str, fn: Callable, args: dict) -> Callable:  # noqa: ARG001
        # Simply call the function with the args
        return fn(**args)

    # Create the image embedder with our classifier
    with (
        mock.patch("nemo_curator.image.embedders.timm.TimmImageEmbedder.load_embedding_model") as mock_load_embedding,
        mock.patch("nemo_curator.image.embedders.timm.TimmImageEmbedder.load_dataset_shard") as mock_load_dataset,
        mock.patch(
        ...
        def mock_load_dataset_shard(tar_path: str) -> Generator[tuple[torch.Tensor, list[dict]], None, None]:  # noqa: ARG001
        ...
        def mock_map_partitions(func: Callable, *args, **kwargs) -> cudf.DataFrame:  # noqa: ARG001
            import cudf
            ...
            mock_result.__iter__ = lambda self: iter([result])  # noqa: ARG005
            return mock_result
        ...
        # Call the embedder
        with mock.patch("nemo_curator.image.embedders.base.ImageTextPairDataset") as mock_dataset_class:
...
@pytest.mark.gpu
def test_nsfw_model_architecture(gpu_client) -> None:  # noqa: ANN001, ARG001
...
@pytest.mark.gpu
def test_normalization_layer(gpu_client) -> None:  # noqa: ANN001, ARG001
...
@pytest.mark.gpu
def test_nsfw_model_with_random_weights(gpu_client) -> None:  # noqa: ANN001, ARG001
...
def test_run_inference_with_mock_model(gpu_client) -> None:  # noqa: ANN001, ARG001
    ...
    class MockNsfwClassifier(NsfwClassifier):
        ...
        def load_model(self, device: torch.device) -> MockModel:  # noqa: ARG002
    ...
    with mock.patch("nemo_curator.image.classifiers.base.load_object_on_worker") as mock_load:
        ...
        assert len(nsfw_scores) == 2  # noqa: PLR2004
        ...
        classifier = MockNsfwClassifier(embedding_column="image_embedding", pred_column="custom_score", batch_size=1)
