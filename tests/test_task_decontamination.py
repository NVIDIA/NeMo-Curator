...
class SimpleTask(DownstreamTask):
    def __init__(self, min_ngram_size=8, max_ngram_size=13):
        ...
        self._dataset = ["This is a simple example task document with enough words.",
...
def contaminated_dataset():
    return list_to_dataset(["This document is fine",
...
class TestPrepareTaskData:
    def test_single_task(self):
        ...
        expected_ngrams = ["this is a simple example task document with enough words",
...
class TestFindMatchingNgrams:
    def test_single_task(self, contaminated_dataset):
        ...
        actual_ngrams, actual_freq = actual_result['matched-ngrams'], actual_result['ngrams-freq']
...
class TestRemoveMatchingNgrams:
    def test_single_task(self, contaminated_dataset):
        ...
        matched_ngrams, ngram_freq = ngram_data['matched-ngrams'], ngram_data['ngrams-freq']
        ...
        expected_data = ["This document is fine",
    ...
    def test_multiple_tasks(self, contaminated_dataset):
        decontaminator = nemo_curator.TaskDecontamination([SimpleTask(), TinyTask()], min_document_length=1, remove_char_each_side=1)
        task_ngrams = decontaminator.prepare_task_ngram_count()
        ngram_data = decontaminator.find_matching_ngrams(task_ngrams, contaminated_dataset)
        matched_ngrams, ngram_freq = ngram_data['matched-ngrams'], ngram_data['ngrams-freq']
        filtered_dataset = decontaminator.remove_matching_ngrams(matched_ngrams, ngram_freq, contaminated_dataset)
