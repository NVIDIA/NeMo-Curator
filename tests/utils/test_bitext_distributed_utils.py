...
class TestBitextWritingFunctions:
    ...
    def test_single_partition_write_to_simple_bitext_nonempty(self, temp_dir: str) -> None:
    ...
    def test_single_partition_write_to_simple_bitext_empty(self, temp_dir: str) -> None:
    ...
    @pytest.mark.gpu
    def test_single_partition_write_to_simple_bitext_with_cudf(self, temp_dir: str) -> None:
    ...
    def test_merge_tmp_simple_bitext_partitions(self, temp_dir: str) -> None:
        ...
        # Create the files
        for i, (en_file, es_file) in enumerate(zip(en_files, es_files, strict=False)):
        ...
        with open(os.path.join(output_dir, "test.en")) as f:
            content = f.read()
            assert content == "".join(en_contents)

        with open(os.path.join(output_dir, "test.es")) as f:
            content = f.read()
            assert content == "".join(es_contents)

    def test_write_to_disk_bitext(self, temp_dir: str) -> None:
        ...
        with (
            ...
            patch("nemo_curator.utils.distributed_utils._merge_tmp_simple_bitext_partitions") as mock_merge,
    ...
    def test_write_to_disk_bitext_with_filename(self, temp_dir: str) -> None:
