...
class TestBuildFilter:
    ...
    def test_build_filter_with_params(self) -> None:
        ...
        mock_filter_instance._name = "test_filter"  # noqa: SLF001
        ...
        with (
            ...
            patch("nemo_curator.ScoreFilter") as mock_score_filter,
        ):
    ...
    def test_build_filter_without_params(self) -> None:
    ...
    def test_build_filter_with_none_params(self) -> None:
    ...
    def test_build_filter_with_log_score(self) -> None:
    ...
    def test_build_filter_filter_only(self) -> None:
        ...
        with (
            ...
            patch("nemo_curator.Filter") as mock_filter,
        ):
...
class TestBuildFilterPipeline:
    ...
    def test_build_filter_pipeline(self) -> None:
        ...
        mock_yaml_load = yaml.load  # noqa: F841
        with (
            patch("builtins.open", mock_open(read_data="dummy")) as mock_file,
            patch("yaml.load", return_value=filter_params) as mock_load,
        ):
            ...
            with (
                ...
                patch("nemo_curator.Sequential") as mock_sequential,
            ):
                ...
                assert mock_build_filter.call_count == 3  # noqa: PLR2004
...
class TestBuildDownloader:
    ...
    def test_build_downloader_with_download_dir(self) -> None:
        ...
        with (
            patch("builtins.open", mock_open(read_data="dummy")) as mock_file,
            patch("yaml.load", return_value=downloader_params) as mock_load,
        ):
            ...
            def mock_locate(type_name: str) -> type | None:
                if type_name == "str":
                    return str
                elif type_name == "int":
                    return int
            ...
            with (
                ...
                patch("nemo_curator.utils.config_utils.expand_outdir_and_mkdir") as mock_expand,
            ):
                # Call the function
                downloader, iterator, extractor, dataset_format = build_downloader("dummy_path.yaml")

                # Check assertions
                mock_file.assert_called_once_with("dummy_path.yaml", "r")
                mock_load.assert_called_once()
                mock_import_downloader.assert_called_once_with("nemo_curator.download.test_downloader")
                mock_import_iterator.assert_called_once_with("nemo_curator.download.test_iterator")
                mock_import_extractor.assert_called_once_with("nemo_curator.download.test_extractor")
                ...
                assert mock_locate_func.call_count == 2  # noqa: PLR2004
    ...
    def test_build_downloader_with_default_download_dir(self) -> None:
        ...
        # Mock the open function and yaml.load
        with patch("builtins.open", mock_open(read_data="dummy")), patch("yaml.load", return_value=downloader_params):
            # Mock the import functions and classes
            mock_downloader_class = MagicMock()
            mock_downloader_instance = MagicMock()
            mock_downloader_class.return_value = mock_downloader_instance
            ...
            with (
                ...
                mock_downloader_class.assert_called_once_with(download_dir=default_dir, param1="value1")

    def test_build_downloader_with_none_download_params(self) -> None:
        ...
        with patch("builtins.open", mock_open(read_data="dummy")), patch("yaml.load", return_value=downloader_params):
            ...
            with (
                ...
                patch("nemo_curator.utils.config_utils.expand_outdir_and_mkdir"),
