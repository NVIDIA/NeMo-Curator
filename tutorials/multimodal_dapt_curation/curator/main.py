...
def process_textual_data(textual_data):
    ...
    for text_data in textual_data:
        ...
        textual_data_list.append([encodings, text, id, file_extension, filename, category, line_count, file_path])
    
    return pd.DataFrame(textual_data_list, columns=["encodings", "text", "id", "file_extension", "file_name", "category", "line_count", "path"])
...
def process_image_data(image_data):
    ...
    for img_data in image_data:
        ...
        image_data_list.append([encodings, text, id, file_extension, filename, category, line_count, file_path])
    
    return pd.DataFrame(image_data_list, columns=["encodings", "text", "id", "file_extension", "file_name", "category", "line_count", "path"])
...
def process_structured_data(structured_data):
    ...
    for struct_data in structured_data:
        ...
        struct_data_list.append([encodings, text, id, file_extension, filename, category, line_count, file_path])
    
    return pd.DataFrame(struct_data_list, columns=["encodings", "text", "id", "file_extension", "file_name", "category", "line_count", "path"])
...
def run_text_curation_pipeline(args: Any, text_ddf, struct_ddf) -> None:
    ...
    text_ddf.df["file_type_count"] = (
        text_ddf.df["category"]
        + " : "
        + text_ddf.df["line_count"].astype(str)
    )

    struct_ddf.df["file_type_count"] = (
        struct_ddf.df["category"]
        + " : "
        + struct_ddf.df["line_count"].astype(str)
    )
