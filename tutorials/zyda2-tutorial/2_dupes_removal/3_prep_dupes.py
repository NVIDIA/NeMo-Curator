...
if __name__ == "__main__":
    ...
    cc_groups_counts_inter_df = cc_grouped_counts_filtered_df[cc_grouped_counts_filtered_df["size"] != cc_grouped_counts_filtered_df["dclm"]]
    cc_groups_counts_inter_df = cc_groups_counts_inter_df[cc_groups_counts_inter_df["size"] != cc_groups_counts_inter_df["fwe2"]]
    cc_groups_counts_inter_df = cc_groups_counts_inter_df[cc_groups_counts_inter_df["size"] != cc_groups_counts_inter_df["dolma-cc"]]
    cc_groups_counts_inter_df = cc_groups_counts_inter_df[cc_groups_counts_inter_df["size"] != cc_groups_counts_inter_df["zyda"]]


    def select_dupes(partition):
        # Removes all overlaps with fwe2
        partition_fwe2 = partition[partition["fwe2"] > 0]
        partition_fwe2["dupes_to_remove"] = partition_fwe2["original_id"].apply(lambda x: json.dumps([id for id in json.loads(x) if "fwe2" not in id]))
        ...
        partition_dclm["dupes_to_remove"] = partition_dclm["original_id"].apply(lambda x: json.dumps([id for id in json.loads(x) if "dclm" not in id]))
        ...
        partition_zyda["dupes_to_remove"] = partition_zyda["original_id"].apply(lambda x: json.dumps([id for id in json.loads(x) if "zyda" not in id]))
    ...
    def group_dupes(partition):
        partition["dclm_dupes"] = partition["dupes_to_remove"].apply(lambda x: json.dumps([id for id in json.loads(x) if "dclm" in id]))
        partition["zyda_dupes"] = partition["dupes_to_remove"].apply(lambda x: json.dumps([id for id in json.loads(x) if "zyda" in id]))
        partition["dolma_dupes"] = partition["dupes_to_remove"].apply(lambda x: json.dumps([id for id in json.loads(x) if "dolma" in id]))

        return partition[["group", "size", "dclm", "fwe2", "dolma-cc", "zyda", "dclm_dupes", "zyda_dupes", "dolma_dupes"]]
