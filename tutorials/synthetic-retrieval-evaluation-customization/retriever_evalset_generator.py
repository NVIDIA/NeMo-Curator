...
class RetrieverEvalSetGenerator(SyntheticDataGenerator):
    ...
    def run(self, dataset: DocumentDataset) -> DocumentDataset:
        ...
        df = dataset.df
        df['llm_response'] = df['text'].apply(self._generate, 
                                              meta=('llm_response', 'str'))
        df['qa_pairs'] = df['llm_response'].apply(self._parse_response, 
                                                   meta=('qa_pairs', 'object'))
        df = df.explode('qa_pairs').reset_index(drop=True)
        df['question'] = df['qa_pairs'].apply(lambda x: x['question'], meta=('question','str'))
        df['answer'] = df['qa_pairs'].apply(lambda x: x['answer'], meta=('answer', 'str'))
