...
class DAPTtxtDownloader(DocumentDownloader):
...
class DAPTtxtIterator(DocumentIterator):
    ...
    def iterate(self, file_path):
        ...
        with gzip.open(file_path, "rt") as file:
            ...
            for line in file:
                if line.strip() == DAPTtxtIterator.SEPARATOR_TOKEN:
                    if example:
...
class DAPTtxtExtractor(DocumentExtractor):
...
class PdfDownloader(DocumentDownloader):
    """
    A class for downloading pdfs from Arxiv
    """
    ...
    def __init__(self, pdf_root_dir: str):
        ...
        self.clone_root_dir = os.path.join(pdf_root_dir, "pdfs")
    ...
    def download(self, url: str) -> str:
        """
        Copy a pdf as a zip file.

        Args:
            url (str): The URL of the pdf.

        Returns:
            str: The path to the downloaded pdf as zip file, or None if the download failed.
        """
        pdf_name = os.path.basename(url)
        zip_file = os.path.join(self.clone_root_dir, pdf_name)
        ...
        if article := next(search_result):
            ...
            pdf_path = article.download_pdf(dirpath=self.clone_root_dir)
            print(f"Download finished! Result saved at:\n{pdf_path}")

            if download_source:
                print(
                    f'Starting download of article source files: "{article.title}" ({article_id})'
                )
                article.download_source(dirpath=self.clone_root_dir)
            success = True
        else:
        ...
        if not success:
            print(
                f"Failed to download pdf '{url}' (error code {response.status_code})."
            )
            return None
...
class PdfIterator(DocumentIterator):
    """
    Pdf document iterator. Will go through the files and parse the supported ones.
    """
    ...
    def iterate(self, file_path: str):
        ...
        # If neither the file extension nor the filename is supported, return None
        if ext not in PdfIterator.SUPPORTED_EXTENSIONS_TO_CATEGORY:
            if filename_no_ext not in PdfIterator.SUPPORTED_EXTENSIONS_TO_CATEGORY:
                return None

            # The filename is there, but the extension is not. The category is determined by the filename.
            category = PdfIterator.SUPPORTED_EXTENSIONS_TO_CATEGORY[filename_no_ext]
        else:
            category = PdfIterator.SUPPORTED_EXTENSIONS_TO_CATEGORY[ext]
        ...
        # Extract the metadata
        line_count = content.count("\n") + 1
        size_in_bytes = os.path.getsize(file_path)  # /(1<<20)
        metadata = {
            # Use the file path as the unique ID
            "id": file_path,
            "file_extension": ext,
