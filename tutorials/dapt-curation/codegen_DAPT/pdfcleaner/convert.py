...
import gzip
import sys
from pdfcleaner.util import dump_text, dehyphenate_page, strip_pos
...
parser.add_argument('--filter', default='academic', choices=[
                        'academic',
                        'manuals',
                    ])
...
def handle(filename):
    ...
    if (not args.clobber) and os.path.isfile(outfile_full_path):
        ...
        outfile_full_path_safe = outfile_full_path.encode('utf-8', 'replace').decode('utf-8')
    ...
    try:
        ...
        print(f"Error {type(e)}: {filename.encode('utf-8', 'replace').decode('utf-8')}. Skipping...")
        return
