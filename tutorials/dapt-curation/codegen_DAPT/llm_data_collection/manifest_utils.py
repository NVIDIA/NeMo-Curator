...
def remove_outliers(data, extension_name):
    ...
    return data[
        (data[extension_name] >= lower_bound) & (data[extension_name] <= upper_bound)
    ]
...
def print_histogram(
    data, extensions, extension_name, remove_outliers=False, do_log=False
):
    ...
    if not filtered_data.empty:
        ...
        if do_log:
            ...
            filtered_data["Log_" + extension_name] = np.log10(
                filtered_data[extension_name]
            )
        ...
        labels = [
            human_readable_range(bins[i], bins[i + 1], do_log) for i in range(num_bins)
        ]

        filtered_data["Hist_Category"] = pd.cut(
            filtered_data[extension_name], bins=bins, labels=labels, include_lowest=True
        )
