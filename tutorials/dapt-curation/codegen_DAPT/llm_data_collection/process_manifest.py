...
def what_is_gen_type(file_path, encoding="ISO-8859-1"):
    with open(file_path, "r", encoding=encoding) as file:
        for i, line in enumerate(file):
            if i > 15:
                break
            if "GENERATED" in line.upper() and (
                "DO NOT EDIT" in line.upper() or "AUTO" in line.upper()
            ):
...
def save_filtered(data, orig_name, subsetname, extensions):
    ...
    if len(data) > 0:
        data.to_csv(
            get_filter_out_path(orig_name, subsetname), index=False, encoding="latin1"
        )
    else:
...
if (
    "nvwiki" in input_file
    or "confluence" in input_file
    or "doc" in input_file
    or "pdf" in input_file
    or "email" in input_file
):
...
for cur_subset in subsets:
    ...
    cur_data = filter_size(
        filter_lines(filter_extensions(data, cur_subset[1]), min_lines, max_lines),
        min_size,
        max_size,
    )
    ...
    if cur_subset[2]:
        ...
        for gen_type in cur_data["GeneratedType"].unique():
            ...
            save_filtered(
                cur_data_gen, input_file, cur_subset[0] + gen_type, cur_subset[1]
            )
    else:
