...
3. **False Positive Check (optional)**: Due to the approximate nature of the bucketing via MinHash + LSH
...
3. **Buckets to Edgelist (Alternative)**: If not using the false positive check, we directly convert the LSH buckets to edges for the connected components computation.
...
All documents in the same group are considered near duplicates. These results be used to remove the near duplicates from the corpus.
