...
class TokenizerFertilityFilter(DocumentFilter):
  ...
  def __init__(self, path_to_tokenizer=None, min_char_to_token_ratio=2.5):
    if path_to_tokenizer is None:
      raise ValueError("Must provide a valid path to a SentencePiece "
                       "tokenizer")
...
class PerExtensionFilter(DocumentFilter):
  ...
  def score_document(self, source):
    ...
    try:
      (include, line_max, line_mean, alphanum_frac, alphabetic_frac) = self._ext_to_filter[(self._language_format_from_dataset(
        self._lang), self._extension if self._extension is not None else ""
      )]
