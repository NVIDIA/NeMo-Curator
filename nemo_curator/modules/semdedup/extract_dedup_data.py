...
def extract_pruned_data(
    ...
    for cluster_id in tqdm(range(num_clusters)):
        ...
        df_cluster_i = pd.DataFrame({'adlr_id': cluster_i[:, 0], 'global_id': cluster_i[:, 1],
                                     'dist': cluster_i[:, 2], 'cluster': cluster_i[:, 3],
                                    })
        ...
        items_to_keep = semdedup_pruning_tables[
            semdedup_pruning_tables[f"eps={eps}"] == False]['global_id'].tolist()        
