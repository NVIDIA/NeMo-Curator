...
class ConnectedComponents:
    ...
    def cc_workflow(self, output_path: str) -> None:
        ...
        return self._run_connected_components(deduped_encoded_jaccard_path, deduped_parsed_id_path, output_path)

    def _run_connected_components(
        self,
        deduped_encoded_jaccard_path: str,
        deduped_parsed_id_path: str,
        output_path: str,
    ) -> None:
        t0 = time.time()
        with performance_report_if_with_ts_suffix(self.profile_dir, "connected-components-run"):
            ...
            multigraph.from_dask_cudf_edgelist(df, source=self.left_id, destination=self.right_id, renumber=False)
            ...
            if num_nodes != len(labels_df):
                msg = f"# of nodes = {num_nodes}, # rows in labels_df = {len(labels_df)}"
    ...
    @staticmethod
    def thresholding(df: cudf.DataFrame, threshold: float, column_to_threshold: str) -> cudf.DataFrame:
        mask = df[column_to_threshold] > threshold
        df.loc[mask, column_to_threshold] = np.int8(1)
        df.loc[~mask, column_to_threshold] = np.int8(0)
        return df

    def _write_dedup_encoded_jaccard_pair(self, encoded_jaccard_pair_path: str) -> str:
    ...
    def _write_dedup_parsed_id(self) -> str:
    ...
    def _write_encoded_jaccard_pair(self, dedup_parsed_id_path: str) -> str:
    ...
    @staticmethod
    def _get_unique_ids_per_partition(df: cudf.DataFrame, id_columns: list[str]) -> cudf.DataFrame:
        unique_df_ls = []
        for tag in ["x", "y"]:
            cols_to_drop = []
