...
# TODO: Combine this with nemo_curator.distributed_utils.read_cudf_jsonl
def _read_json_func(
    files: list[str],
    engine: str = "cudf",
...
def get_text_ddf_from_json_path_with_blocksize(  # noqa: PLR0913
    ...
    return text_ddf.map_partitions(
...
def get_bucket_ddf_from_parquet_path(
...
def aggregated_anchor_docs_with_bk_read(path: str, blocksize: int) -> dd.DataFrame:
    ...
    if len(chunks) == 0:
        msg = "No chunks found in the dataset"
...
def get_restart_offsets(output_path: str) -> tuple[int, int]:
...
def update_restart_offsets(output_path: str, bucket_offset: int, text_offset: int) -> None:
