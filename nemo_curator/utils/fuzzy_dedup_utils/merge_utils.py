...
def _split_part(part, nsplits):
    ...
    return {i: part.iloc[offset : offset + stride] for i, offset in enumerate(range(0, total_rows, stride))}


def text_bytes_aware_merge(text_df, right_df, broadcast=True, *, on):
    if query_planning_enabled():
        raise NotImplementedError(
            "The text_bytes_aware_merge function is not supported when query-planning is enabled."
        )
    ...
    string_size["text"] = string_size["text"].map_partitions(lambda s: s.str.byte_count())
    post_merge_size = (
