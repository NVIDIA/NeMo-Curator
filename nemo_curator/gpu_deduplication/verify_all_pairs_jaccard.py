...
def write_eligible_pairs(dedup_with_text_path, cache_dir):
  ...
  for s in range(0, df.shape[0], B):
    ...
    db = db[mask]
    mask = (db["num_ngram_x"] < db["num_ngram_y"] *
            0.8) | (db["num_ngram_y"] < db["num_ngram_x"] * 0.8)
...
def get_max_num_rows_to_process_once(df):
  nbytes = max(df["text_x"].str.byte_count().sum(),
               df["text_y"].str.byte_count().sum())
...
def run_verify_all_pairs_jaccard(dedup_with_text_path, cache_dir, output_dir):
  ...
  ddf = ddf.map_partitions(partial(merge_text,
                                   dedup_with_text_path=dedup_with_text_path),
                           meta=meta_df)
