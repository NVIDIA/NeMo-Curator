...
def batched_merge_and_write(
  ...
  for bucket_part_offset in tqdm(
      range(bucket_part_start_offset, bucket_part_end_offset,
            parts_per_bucket_batch)):

    # Outer loop over batches of "bucket-map" partitions
    end_bucket_offset = min(bucket_part_offset + parts_per_bucket_batch,
    ...
    while text_part_offset < text_part_end_offset:
      ...
      end_text_offset = min(text_part_offset + parts_per_text_batch_use,
...
def jaccard_shuffling_workflow(
  ...
  print(
      "Graph creation for get_text_ddf_from_json_path_with_blocksize"
      " complete.",
      flush=True)
  ...
  dask_profile_name = (dask_profile_name +
