...
def main(args):
    ...
    for file_names in get_batched_files(
        ...
        source_data = read_data(file_names, file_type=args.input_file_type, backend='pandas', add_filename=True)
    ...
    end_time = time.time()
    logging.debug("Total time taken in PII job: %0.3f seconds" % (end_time - start_time))
