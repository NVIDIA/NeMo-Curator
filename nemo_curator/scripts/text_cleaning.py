...
def main(args):
  ...
  cleaner = nemo_curator.Modify(UnicodeReformatter(), text_field=args.input_text_field)

  for files in get_batched_files(args.input_data_dir, output_clean_dir, args.input_file_type, batch_size=args.batch_size):
    dataset = DocumentDataset(read_data(files, file_type=args.input_file_type, backend="pandas", add_filename=True))
    cleaned_dataset = cleaner(dataset)
    write_to_disk(cleaned_dataset.df, output_clean_dir, write_to_filename=True, output_type=args.output_file_type)
