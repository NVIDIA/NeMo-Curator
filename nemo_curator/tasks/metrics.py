...
class MMLU(DownstreamTask):
  ...
  def __init__(self, path=None, min_ngram_size=8, max_ngram_size=13):
    ...
    if self._path is None:
      raise Exception("Must provide path that contain "
                      "MMLU task data in JSONL format")
...
class Multilingual(DownstreamTask):
  ...
  def __init__(self, path=None, min_ngram_size=8, max_ngram_size=13):
    ...
    self._path = path
    if self._path is None:
      raise Exception("Must provide path to "
                      "multilingual task data in JSONL format")
    ...
    with open(self._file_path, 'r') as f:
      for line in f:
        try:
          myjson = json.loads(line)
          text = " ".join([
              myjson["InputSentence1"], myjson["InputSentence2"],
              myjson["InputSentence3"], myjson["InputSentence4"]
          ])
          self._update_ngrams(text, self._min_ngram_size, self._max_ngram_size)
        except Exception as e:
          print(f"Error {e}")
