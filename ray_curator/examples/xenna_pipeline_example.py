...
class HtmlExtractorStage(ProcessingStage[DocumentBatch]):
    ...
    def process(self, task: DocumentBatch) -> Optional[DocumentBatch]:
...
def create_example_pipeline():
    ...
    initial_tasks = [
        FileGroupTask(
            ...
            file_paths=["data/file1.jsonl", "data/file2.jsonl"],
            metadata={"group": 1}
        ),
        FileGroupTask(
            task_id="file_group_2",
            dataset_name="example_dataset",
            file_paths=["data/file3.jsonl", "data/file4.jsonl"],
            metadata={"group": 2}
        ),
    ]
    ...
    execution_plan = ExecutionPlan(
        pipeline_name="text_processing_example",
        stages=stages,
