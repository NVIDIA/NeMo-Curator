...
from ray_curator.data import DocumentBatch, Task
...
class TextPreprocessingPipeline(CompositeStage):
    ...
    def decompose(self) -> list[ProcessingStage]:
        ...
        stages.append(
            TextFilterStage(min_length=self.min_length, max_length=self.max_length, name=f"{self.name}_filtering")
        )
...
def create_example_pipeline():
    ...
    pipeline = Pipeline(name="text_processing_example", description="Example using composite stages")

    # Add the high-level preprocessing stage
    # Users only need to configure high-level parameters
    preprocessing = TextPreprocessingPipeline(min_length=50, max_length=10000, include_language_detection=True)
...
def demonstrate_decomposition():
    ...
    composite = TextPreprocessingPipeline(min_length=100, max_length=5000, include_language_detection=True)
